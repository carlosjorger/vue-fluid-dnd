...
description: A guide on how to use Vue Fluid DnD for removing elements on a list.
---
...
To showing how to use **Vue Fluid DnD** for removing elements on a list.
First, we're going to create a list and extract the removeAt function:
...
After, we’ll create the list with an remove button that call the `removeAt`:

export const highlightsDroppable = ["removeAt"];
...
You can add the css class setted when an element is removed:
...
export const highlightsRemovingClass = ["removingClass"];
